...
pub fn setup(mut commands: Commands, asset_server: Res<AssetServer>,
    ...
    let ldtk_handle = asset_server.load("levels/test_level.ldtk");
    commands.spawn(LdtkWorldBundle {
        ldtk_handle,
        ..Default::default()
    });

    // (doesn't work) Fix player sprite offset
    for mut sprite in q_player.iter_mut() {
        info!("FIX PLAYER SPRITE OFFSET");
        sprite.anchor = bevy::sprite::Anchor::Custom(Vec2::new(0., 50.));
    }
}
...
pub fn movement(
    ...
    for (mut velocity, mut climber, ground_detection) in &mut query {
        ...
        velocity.linvel.x = (right - left) * PLAYER_SPEED * delta_time;
